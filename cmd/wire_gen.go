...
func wireApp(configConfig *config.Config) (*app.App, func(), error) {
	...
	cacheAdminDao := dao.NewCacheAdminDao(fileDao)
	cacheAdminService := service.NewCacheAdminService(cacheAdminDao)
	cacheAdminHandler := handler.NewCacheAdminHandler(cacheAdminService)
